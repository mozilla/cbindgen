...
impl Type {
    ...
    pub fn is_volatile(&self) -> bool {
        match *self {
            Type::Ptr { is_volatile, .. } => is_volatile,
            Type::Path { is_volatile, .. } => is_volatile,
            Type::Primitive { is_volatile, .. } => is_volatile,
            Type::FuncPtr { is_volatile, .. } => is_volatile,
            _ => false,
        }
    }
        ...
        match *self {
            ...
            Type::Path {
                ...
                if let Some(items) = library.get_items(path) {
                    for item in items {
                        item.deref()
                            .instantiate_monomorph(generic_path.generics(), library, out);
                    }
                }
            }
            Type::Primitive { .. } => {}
