...
impl Enum {
    ...
    pub(crate) fn write_tag_field<F: Write>(
        ...
        {
            let binding = self.annotations.atom("tag-field-name");
            let tag_field_name = binding
                .as_ref()
                .map_or("tag", |tag_field_name| tag_field_name.as_ref().unwrap());

            write!(out, "{} {};", tag_name, tag_field_name);
        }
