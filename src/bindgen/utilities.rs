...
pub trait SynAttributeHelpers {
    ...
    /// Looks up the `#[cbindgen::namespace = "..."]` attribute and parses it
    /// into a vector of namespace segments. Supports `::` as the separator.
    ...
    fn get_cbindgen_namespace(&self) -> Option<Vec<String>> {
        for attr in self.attrs() {
            ...
            {
                // Check for #[cbindgen::namespace = "..."]
                if path.segments.len() == 2 {
                    let first = &path.segments[0];
                    let second = &path.segments[1];
                    if first.ident == "cbindgen" && second.ident == "namespace" {
                        let value = lit.value();
                        if value.is_empty() {
                            return Some(Vec::new());
                        }
