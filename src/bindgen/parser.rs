...
impl Parse {
    ...
    pub fn extend_with(&mut self, other: &Parse) {
        ...
        self.assoc_types = other.assoc_types.clone();
    }
    ...
    fn load_syn_impl(&mut self, item_impl: &syn::ItemImpl) {
        ...
        for impl_item in &item_impl.items {
            if let syn::ImplItem::Type(impl_item_type) = impl_item {
                ...
                let output =
                    Parse::find_dependings_assoc_types(&mut self.assoc_types, &mut conc_type);
    ...
    fn find_dependings_assoc_types(
        ...
        match concrete_type {
            Type::Ptr { ty: ty_, .. } => {
                Parse::find_dependings_assoc_types(assoc_types, &mut **ty_)
            }
            Type::Array(ty_, _) => Parse::find_dependings_assoc_types(assoc_types, &mut **ty_),
            Type::FuncPtr { ret, args, .. } => args
                .iter_mut()
                .map(|(_, ty_)| Parse::find_dependings_assoc_types(assoc_types, ty_))
                .flatten()
